...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function syncSession(currentUser) {
      ...
      const canOperate = ["owner", "admin", "operator"].includes(normalizedRole);
      const canManage = ["owner", "admin"].includes(normalizedRole);

      const identity = {
        nome: profileRow?.nome || currentUser.email || "Usuário",
      ...
      setAuthState({
        inventario: canOperate ? identity : null,
        materiais: canManage || canOperate ? identity : null,
      });
...
async function handleLogin() {
    if (!supabase) {
      setLoginError("Supabase não encontrado.");
      return;
    }
    ...
    if (!identifier.includes("@")) {
      const { data: foundProfiles, error: profileLookupError } = await supabase
        .from("profiles")
        .select("email")
      ...
      if (profileLookupError) {
        setLoginError(profileLookupError.message || "Erro ao localizar usuário.");
        return;
      }
      ...
      resolvedEmail = foundProfiles[0].email;
    }
  ...
  async function saveMaterial() {
    ...
    const actor = authState.materiais || authState.inventario || { nome: currentUserName, matricula: profile?.matricula || "-" };
  ...
  async function saveCurrentInventory(options = { silent: false }) {
    ...
    try {
      const isNew =
        !currentInventory.id || String(currentInventory.id).startsWith("temp-");
      ...
      if (isNew) {
        const { data, error } = await supabase
          ...
          .insert({
            ...
            criado_por:
              currentInventory.criadoPor || currentUserName,
      ...
      const savedInventory =
        inventories.find((inv) => inv.id === inventoryId) || null;
  ...
  async function registerMovement() {
    ...
    const actor = authState.materiais || { nome: currentUserName, matricula: profile?.matricula || "-" };
  ...
  useEffect(() => {
    ...
    if (
      selectedMaterialPn &&
      !filteredMaterials.some((item) => item.pn === selectedMaterialPn)
    ) {
      setSelectedMaterialPn(filteredMaterials[0]?.pn || "");
    }
    ...
    return (
      ...
      >
        ...
        >
          ...
          <div style={{ background: "white", padding: 36, display: "grid", alignItems: "center" }}>
            <div>
              ...
              <div style={{ marginTop: 24, display: "grid", gap: 14 }}>
                <Field label="Email ou matrícula">
                  <TextInput
                    value={loginEmail}
                ...
                <ActionButton onClick={handleLogin} fullWidth>
                  Acessar plataforma
                </ActionButton>
              </div>
            </div>
      ...
      >
        ...
        >
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    boxShadow: isActive
                      ? "0 10px 24px rgba(47,111,237,0.18)"
                      : "none",
                  }}
                >
                  <span
          ...
          >
            ...
            >
              <div style={{ color: "#94A3B8", fontSize: 11, letterSpacing: 0.4 }}>
                MATERIAIS
              </div>
          ...
          >
            ...
            <ActionButton kind="secondary" onClick={handleLogout} fullWidth>
              Sair da plataforma
            </ActionButton>
          </div>
        </aside>
          ...
          >
            ...
            <div style={{ marginTop: 18 }}>
                  ...
                  <Panel
                    ...
                    right={
                      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                        ...
                        <ActionButton kind="secondary" onClick={loadCloudData}>
                          Atualizar nuvem
                        </ActionButton>
                      </div>
                    }
              ...
              {activeTab === "inventario" && !canAccessInventario && (
                <Panel title="Área de Inventário">
                  <div style={{ border: "1px dashed #cbd5e1", borderRadius: 14, background: "#f8fafc", padding: 32, textAlign: "center", color: "#64748b" }}>
                    Seu perfil não tem permissão para acessar o inventário.
                  </div>
                </Panel>
              )}
                ...
                <Panel
                  ...
                  right={
                    <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                      ...
                      <ActionButton kind="secondary" onClick={logoutArea}>
                        Sair
                      </ActionButton>
                    </div>
                  }
              ...
              {activeTab === "cadastro" && !canAccessMateriais && (
                <Panel title="Cadastro de materiais">
                  <div style={{ border: "1px dashed #cbd5e1", borderRadius: 14, background: "#f8fafc", padding: 32, textAlign: "center", color: "#64748b" }}>
                    Seu perfil não tem permissão para acessar o cadastro de materiais.
                  </div>
                </Panel>
              )}
                    ...
                    <div style={{ display: "grid", gap: 14 }}>
                      ...
                      <Field label="PN">
                        <TextInput
                          ...
                          onChange={(e) =>
                            setMovementForm((prev) => ({ ...prev, pn: e.target.value }))
                          }
                          placeholder="Digite o PN"
                        />
                      </Field>
                      ...
                      {matchedMovementMaterial ? (
                        ...
                        >
                          ...
                          <div style={{ fontSize: 14, color: "#6b7280", marginTop: 4 }}>
                            Localização:{" "}
                            {matchedMovementMaterial.localizacao || "Não informada"}
                          </div>
                          <div style={{ fontSize: 14, color: "#6b7280", marginTop: 4 }}>
                            Quantidade atual cadastrada:{" "}
                            {matchedMovementMaterial.quantidade ?? 0}
                          </div>
                        </div>
                      ) : null}
                          ...
                          .map((item) => (
                            ...
                            >
                              ...
                              >
                                ...
                                >
                                  <div style={{ fontSize: 12, color: "#6b7280" }}>Antes</div>
                                  <div style={{ fontWeight: 700, marginTop: 6 }}>
                                    {item.quantidadeAntes}
                                  </div>
                                </div>
                                ...
                                >
                                  <div style={{ fontSize: 12, color: "#6b7280" }}>Depois</div>
                                  <div style={{ fontWeight: 700, marginTop: 6 }}>
                                    {item.quantidadeDepois}
                                  </div>
                                </div>
